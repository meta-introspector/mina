...
module Config = struct
  ...
  let to_args t = 
    ...
    in
    let maybe_output_folder = match t.output_folder with 
      | Some folder -> ["--output-folder" ; folder]
      | None -> []
    in
    let maybe_network = match t.network with 
      | Some network -> ["--network" ; network]
      | None -> []
    in
    let maybe_include_block_height_in_name = match t.include_block_height_in_name with 
      | true -> ["--include-block-height-in-name" ]
      | false -> []
    in

    ["--archive-uri"; t.archive_uri] @ maybe_output_folder @ maybe_network @ maybe_include_block_height_in_name @ blocks 
